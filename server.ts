...
const dbPath = path.join(process.cwd(), 'maritime.db');
...
const existingAdmin = db.prepare('SELECT * FROM users WHERE email = ?').get(adminEmail) as UserRow | undefined;
if (!existingAdmin) {
...
const commonAdmin = db.prepare('SELECT * FROM users WHERE userId = ?').get('100') as UserRow | undefined;
if (!commonAdmin) {
...
app.delete('/api/operations/:id', (req, res) => {
  ...
  const op = db.prepare('SELECT o.*, s.name as shipName, b.billNumber, c.carNumber, u.name as employeeName FROM operations o JOIN ships s ON o.shipId = s.id JOIN bills b ON o.billId = b.id JOIN cars c ON o.carId = c.id JOIN users u ON o.employeeId = u.id WHERE o.id = ?').get(req.params.id) as OperationRow | undefined;
  if (op) {
...
app.post('/api/restore-operation/:id', (req, res) => {
    ...
    const log = db.prepare('SELECT * FROM deleted_operations WHERE id = ?').get(req.params.id) as DeletedOperationRow | undefined;
    if (!log) return res.status(404).json({ success: false });
    
    const ship = db.prepare('SELECT id FROM ships WHERE name = ?').get(log.shipName) as { id: number } | undefined;
    const bill = db.prepare('SELECT id FROM bills WHERE billNumber = ?').get(log.billNumber) as { id: number } | undefined;
    const car = db.prepare('SELECT id FROM cars WHERE carNumber = ?').get(log.carNumber) as { id: number } | undefined;
    const user = db.prepare('SELECT id FROM users WHERE name = ?').get(log.employeeName) as { id: number } | undefined;
...
app.post('/api/restore', (req, res) => {
  ...
  // Map sheet names to table names if necessary (case-insensitive)
  const tableMapping: Record<string, string> = {
    'customs': 'customs_data',
    'freezone': 'freezone_data',
  };

  const normalizedTable = table.toLowerCase();
  const targetTable = tableMapping[normalizedTable] || table;

  console.log(`Starting restore for table: ${targetTable} (from sheet: ${table})`);
  ...
  try {
    ...
    const tableInfo = db.prepare(`PRAGMA table_info("${targetTable}")`).all() as TableColumn[];
    if (tableInfo.length === 0) {
      console.warn(`Table ${targetTable} does not exist, skipping restore for this sheet.`);
      return res.json({ success: true, message: `Table ${targetTable} does not exist, skipped.`, skipped: true });
    }

    if (records.length === 0) {
      return res.json({ success: true, message: `No records to restore for ${targetTable}` });
    }

    const dbColumns = tableInfo.map(c => c.name);
    ...
    const transaction = db.transaction((data) => {
      ...
      // Find columns that exist in both the input data and the database (case-insensitive)
      const inputKeys = Object.keys(data[0]);
      const columnsToInsert: string[] = [];

      inputKeys.forEach(inputKey => {
        const dbColumn = dbColumns.find(c => c.toLowerCase() === inputKey.toLowerCase());
        if (dbColumn) {
      ...
      if (columnsToInsert.length === 0) {
        throw new Error(`لم يتم العثور على أعمدة متطابقة في الجدول ${targetTable}`);
      }

      const placeholders = columnsToInsert.map(() => '?').join(', ');
      const sql = `INSERT INTO "${targetTable}" (${columnsToInsert.map(c => `"${c}"`).join(', ')}) VALUES (${placeholders})`;
      const insert = db.prepare(sql);

      for (const record of data) {
        const values = columnsToInsert.map(col => {
          const inputKey = inputKeys.find(k => k.toLowerCase() === col.toLowerCase());
          return inputKey ? record[inputKey] : null;
        });
        insert.run(...values);
      }
    });
    ...
    transaction(records);
    console.log(`Successfully restored ${records.length} records to ${targetTable}`);
...
app.get('/api/instructions', (req, res) => {
  ...
  const user = db.prepare('SELECT jobTitle FROM users WHERE id = ?').get(user_id) as UserRow | undefined;
...
app.get('/api/snapshots/create-and-download', async (req, res) => {
  const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
  const filename = `maritime_backup_${timestamp}.db`;
  ...
  try {
    ...
    res.status(500).json({ success: false, message: `تعذر إنشاء ملف النسخة: ${error.message}` });
  }
});

app.post('/api/snapshots/upload-restore', upload.single('file'), async (req, res) => {
  if (!req.file) {
    return res.status(400).json({ success: false, message: 'لم يتم رفع أي ملف' });
  }

  const filePath = req.file.path;
  console.log(`Restoring from uploaded snapshot: ${filePath}`);

  try {
    if (!fs.existsSync(filePath)) {
      throw new Error('الملف المرفوع غير موجود على الخادم');
    }
    const stats = fs.statSync(filePath);
    console.log(`Uploaded file size: ${stats.size} bytes`);

    const sourceDb = new Database(filePath, { readonly: true });
    console.log('Opened source database connection');

    db.pragma('wal_checkpoint(TRUNCATE)');
    console.log('Checkpoint completed on main database');
    
    // @ts-ignore - The backup method supports a Database instance
    ...
    console.error('Snapshot upload-restore error details:', error);
    if (fs.existsSync(filePath)) {
      try { fs.unlinkSync(filePath); } catch (e) {}
    }
    res.status(500).json({ success: false, message: `فشل استعادة النظام: ${error.message}` });
  }
});

app.post('/api/snapshots', async (req, res) => {
  console.log('API POST /api/snapshots received');
  const { name } = req.body;
  const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
  const filename = `${name || 'backup'}_${timestamp}.db`;
  const filePath = path.join(backupsDir, filename);

  console.log(`Creating snapshot: ${filename} at ${filePath}`);
  try {
    // Flush WAL to disk before backup
    db.pragma('wal_checkpoint(TRUNCATE)');
    ...
    if (fs.existsSync(filePath) && fs.statSync(filePath).size > 0) {
      console.log(`Snapshot created successfully: ${filePath}`);
      res.json({ success: true, filename });
    } else {
      throw new Error('فشل إنشاء ملف النسخة الاحتياطية أو الملف فارغ');
    }
  } catch (error: any) {
    console.error('Snapshot creation error:', error);
    res.status(500).json({ success: false, message: `تعذر إنشاء النسخة: ${error.message}` });
  }
});

app.post('/api/snapshots/restore/:filename', async (req, res) => {
  ...
  try {
    // Open the snapshot file as a separate database
    const sourceDb = new Database(filePath, { readonly: true });
    
    // Flush current DB WAL
    db.pragma('wal_checkpoint(TRUNCATE)');

    // Use the backup API to restore contents into the live DB
    // @ts-ignore - The backup method supports a Database instance
    await sourceDb.backup(db);
    sourceDb.close();
    
    console.log(`Snapshot restored successfully via backup API`);
    res.json({ success: true });
  } catch (error: any) {
...
app.get('/api/snapshots/download/:filename', (req, res) => {
  const { filename } = req.params;
  const filePath = path.join(backupsDir, filename);
